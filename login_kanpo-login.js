...
fs.access(path.join(userHome, '.login_kanpo_env.json'), function (err) {
  if (err) {
    console.log('execute `login_kanpo setting`')
    process.exit();
  }
});

var setting = require(path.join(userHome, '.login_kanpo_env.json'));
